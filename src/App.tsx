...
export const App: React.FC = () => {
  // Dark mode intentionally deferred for current release — light theme only (see MOBILE_PREMIUM_UX instructions §1B).
  // Keep CSS dark: tokens for future re-enablement; do not activate .dark at runtime.
  useEffect(() => {
    const root = document.documentElement;
    root.classList.remove('dark');
    // Clear any previously persisted dark preference so system dark does not re-activate later
    try {
      const stored = safeLocalStorage.getItem('codepackr_theme');
      if (stored === 'dark') safeLocalStorage.setItem('codepackr_theme', 'light');
    } catch { /* ignore */ }
  }, []);
  ...
  const renderTool = (tool: ToolDef) => {
    ...
    if (tool.id === 'json-definition-generator') toolViewContent = <JsonDefinitionView tool={tool} onBackToHome={navigateBack} onSelectRelated={navigateToTool} initialInput={initialInputForTool} />;
      ...
      switch (tool.category) {
        ...
        default: toolViewContent = <div className="p-8 text-center text-[color:var(--ink-muted)]">Tool view not found for {tool.id}</div>;
      }
    }
